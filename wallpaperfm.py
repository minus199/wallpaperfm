...
def usage():
    ...
    print ("--------------")
    print (
    "./wallpaperfm.py -m tile -u your_lastfm_username	will generate an image with all your favorite albums tiled up in a random order.")
    print (
    "./wallpaperfm.py -m glass -u your_lastfm_username	will generate an image with a small random collection of albums, with a glassy effect.")
    print (
    "./wallpaperfm.py -m collage -u your_lastfm_username	will generate a random collage of your favorite albums.")
    ...
    print (
    "-x, --ExcludedList: ['http://cdn.last.fm/depth/catalogue/noimage/cover_med.gif'] excluded urls, comma separated.")
    print (
    "-l, --Local: use a local copy of the charts. Ideal for using it offline or being kind to the last.fm servers.")
...
def getParameters():
    ...
    try:
        optlist, args = getopt(sys.argv[1:], 'hu:t:n:c:f:a:o:g:O:i:m:p:s:e:d:r:x:l',
                               ["help", "Mode=", "Username=", "Past=", "Filename=", "CanvasSize=", "ImageSize=",
                                "FinalOpacity=", "AlbumSize=", "AlbumOpacity=", "GradientSize=", "Passes=",
                                "AlbumNumber=", "Interspace=", "Cache=", "Offset=", "EndPoint=", "ExcludedList=",
                                "Local"])
...
def getAlbumCovers(Username='Koant', Past='overall', cache='wp_cache',
                   ExcludedList=['http://cdn.last.fm/depth/catalogue/noimage/cover_med.gif',
                                 'http://cdn.last.fm/flatness/catalogue/noimage/2/default_album_medium.png'], Limit=50,
                   Local='no'):
...
def Tile(Profile, ImageSize=(1280, 1024), CanvasSize=(1280, 1024), AlbumSize=130, FinalOpacity=30, Interspace=5):
    ...
    Profile['Limit'] = ny * nx + len(
        Profile['ExcludedList']) + 5  # some extra in case of 404 , even though there shouldn't be any really.
    ...
    for j in range(0, ny):
        ...
        for i in range(0, nx):
            ...
            if len(
                    filelist2) == 0:  # better than random.choice() (minimises risk of doubles and goes through the whole list)
            ...
            try:
                ...
                print (
                "You might want to exclude its corresponding URL with -x because it probably doesn't point to an image.")
                print ("#" * 20)
                sys.exit()
...
def Glass(Profile, ImageSize=(1280, 1024), CanvasSize=(1280, 1024), AlbumNumber=7, FinalOpacity=100, Offset=50,
          EndPoint=75):
...
def makeCollageMask(size, transparency, gradientsize):
    ...
    for i in range(sizex):
        for j in range(sizey):
            v = c * (erfc(s2 * (l - i)) - erfc(s2 * (sizex - l - i))) * (
            erfc(s2 * (l - j)) - erfc(s2 * (sizex - l - j)))
            mask.putpixel((i, j), int(v))

    return mask


def Collage(Profile, ImageSize=(1280, 1024), CanvasSize=(1280, 1024), AlbumNumber=50, AlbumSize=300, GradientSize=20,
            AlbumOpacity=70, Passes=4, FinalOpacity=70):
